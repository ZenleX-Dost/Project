...
class ImprovedGenerator(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = checkpoint(self.initial, x, use_reentrant=False)    #changehere
        x2 = self.down1(x1)
        x3 = self.down2(x2)
        x = self.down3(x3)
...
class ImprovedVGGLoss(nn.Module):
    def __init__(self):
        super().__init__()
        vgg = vgg19(weights=models.VGG19_Weights.IMAGENET1K_V1)
    ...
    def __init__(self, sketch_dir, image_dir, mode='train', image_size=256):
        ...
        if mode == 'train':
            self.transform = A.Compose([
                ...
                A.HorizontalFlip(p=0.5), # New augmentation
                ...
                A.HueSaturationValue(p=0.3),  # Adjust colors
                ToTensorV2(),
            ])
...
def compute_discriminator_loss(discriminator, real_images, fake_images, sketches, lambda_gp=10):
    ...
    # Balance the gradient penalty with a factor of 5
    d_loss += 5 * lambda_gp * gp
    return d_loss
...
def train(config, resume_from=None):
    ...
    dataset = SketchDataset(
        sketch_dir="C:/Users/amine/Desktop/Sketch2Image/data/sketches",
        image_dir="C:/Users/amine/Desktop/Sketch2Image/data/images",     
    ...
    scheduler_g = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_g, T_max=10, eta_min=1e-6)
    scheduler_d = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer_g, T_max=10, eta_min=1e-6)
